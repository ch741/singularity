...
# are identical. Otherwise there will be an OS or version mismatch
...
if [ -n $SINGULARITY_BOOTSTRAP ]; then
    ...
    awk '/^ *BootStrap: *.*/ && count++ {
        sub("^ *BootStrap: *.*","")}{print}' "$SINGULARITY_TMPDEF" > tmp && \
    mv tmp $SINGULARITY_TMPDEF
else
...
if [ -n $OSVERSION ]; then
    ...
    awk '/^ *OSVersion: *.*/ && count++ {
    sub("^ *OSVersion: *.*","")}{print}' "$SINGULARITY_TMPDEF" > tmp && \
        mv tmp $SINGULARITY_TMPDEF
fi
...
awk '/^ *MirrorURL: *.*/ && count++ {
    sub("^ *MirrorURL: *.*","")}{print}' "$SINGULARITY_TMPDEF" > tmp && \
mv tmp $SINGULARITY_TMPDEF

UPDATEURL=`sed -n -e 's/UpdateURL:\ //g' "$SINGULARITY_TMPDEF"|sort|uniq`
awk '/^ *UpdateURL: *.*/ && count++ {
    sub("^ *UpdateURL: *.*","")}{print}' "$SINGULARITY_TMPDEF" > tmp && \
mv tmp $SINGULARITY_TMPDEF

DUPPKGS=`sed -n -e 's/Include:\ //gp' "$SINGULARITY_TMPDEF"|tr ' ' '\n'|sort|uniq|tr '\n' ' '`
awk '/^ *Include: *.*/ && count++ {
    sub("^ *Include: *.*","")}{print}' "$SINGULARITY_TMPDEF" > tmp && \
mv tmp $SINGULARITY_TMPDEF
